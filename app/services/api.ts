...
> => {
  try {
    ...
    // Fix 4: Properly handle AbortError and other error types
    if (error instanceof Error) {
      if (error.name === 'AbortError') {
        console.warn("Session history fetch timed out - falling back to local data");
      } else {
    ...
    // Fix 4: Always return empty array as fallback to prevent crashes
    return [];
  }
};
